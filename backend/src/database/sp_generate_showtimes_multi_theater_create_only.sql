-- ============================================
-- Multi-theater Wrapper SP (ROUTINE EDITOR MODE)
...
BEGIN
    ...
    theater_loop: WHILE v_theater_csv IS NOT NULL AND LENGTH(TRIM(v_theater_csv)) > 0 DO
        ...
        CALL sp_generate_showtimes(41, v_theater_id, p_start_date, p_end_date, 138, 15, v_aud_g1, '09:00,12:00,15:00,18:00,21:00', v_inserted, v_skipped);
        ...
        CALL sp_generate_showtimes(42, v_theater_id, p_start_date, p_end_date, 107, 15, v_aud_g2, '09:30,11:45,14:00,16:15,18:30,20:45', v_inserted, v_skipped);
        SET v_total_inserted = v_total_inserted + COALESCE(v_inserted, 0);
        SET v_total_skipped = v_total_skipped + COALESCE(v_skipped, 0);
        INSERT INTO temp_showtime_seed_summary VALUES (v_theater_id, 'ZOOTOPIA 2 (42) - 2D', v_inserted, v_skipped);

        CALL sp_generate_showtimes(40, v_theater_id, p_start_date, p_end_date, 103, 15, v_aud_g3, '10:00,12:00,14:00,16:00,18:00,20:00,22:00', v_inserted, v_skipped);
        SET v_total_inserted = v_total_inserted + COALESCE(v_inserted, 0);
        SET v_total_skipped = v_total_skipped + COALESCE(v_skipped, 0);
        INSERT INTO temp_showtime_seed_summary VALUES (v_theater_id, 'TRUY TIM (40) - 2D', v_inserted, v_skipped);

        CALL sp_generate_showtimes(39, v_theater_id, p_start_date, p_end_date, 110, 15, v_aud_g4, '10:20,12:40,15:00,17:20,19:40,22:00', v_inserted, v_skipped);
        SET v_total_inserted = v_total_inserted + COALESCE(v_inserted, 0);
        SET v_total_skipped = v_total_skipped + COALESCE(v_skipped, 0);
        ...
        IF v_aud_imax IS NOT NULL AND TRIM(v_aud_imax) <> '' THEN
            CALL sp_generate_showtimes(41, v_theater_id, p_start_date, p_end_date, 138, 15, v_aud_imax, '09:00,12:00,15:00,18:00,21:00', v_inserted, v_skipped);
            SET v_total_inserted = v_total_inserted + COALESCE(v_inserted, 0);
            SET v_total_skipped = v_total_skipped + COALESCE(v_skipped, 0);
        ...
        IF v_aud_4dx IS NOT NULL AND TRIM(v_aud_4dx) <> '' THEN
            CALL sp_generate_showtimes(42, v_theater_id, p_start_date, p_end_date, 107, 15, v_aud_4dx, '09:30,11:45,14:00,16:15,18:30,20:45', v_inserted, v_skipped);
            SET v_total_inserted = v_total_inserted + COALESCE(v_inserted, 0);
            SET v_total_skipped = v_total_skipped + COALESCE(v_skipped, 0);
        ...
        IF v_aud_screenx IS NOT NULL AND TRIM(v_aud_screenx) <> '' THEN
            CALL sp_generate_showtimes(39, v_theater_id, p_start_date, p_end_date, 110, 15, v_aud_screenx, '10:20,12:40,15:00,17:20,19:40,22:00', v_inserted, v_skipped);
            SET v_total_inserted = v_total_inserted + COALESCE(v_inserted, 0);
            SET v_total_skipped = v_total_skipped + COALESCE(v_skipped, 0);
