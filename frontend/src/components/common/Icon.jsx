...
// Icon library for common UI icons
const icons = {
  user: (
  ...
  diamond: (
    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
      <path d="M6 2L2 8L12 22L22 8L18 2H6M12 5.5L9.5 8H14.5L12 5.5M8.5 10L12 19L15.5 10H8.5M6.5 8L4 10L6.5 8M17.5 8L20 10L17.5 8Z" stroke="currentColor" strokeWidth="0.5"/>
      <path d="M6 2L12 8M18 2L12 8M2 8L12 18M22 8L12 18" stroke="currentColor" strokeWidth="0.3" opacity="0.5"/>
    </svg>
  ),
  gift: (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
...
export default function Icon({ name, className = 'w-5 h-5', size, children, ariaHidden = true }) {
  // If children are provided, render them (backward compatibility)
  if (children) {
    return (
      <span className={`inline-block ${size ? '' : className}`} style={size ? { width: size, height: size } : {}} aria-hidden={ariaHidden}>
        {children}
      </span>
    );
  }

  // Otherwise, use the icon library
